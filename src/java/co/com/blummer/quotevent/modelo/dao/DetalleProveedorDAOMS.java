        ...
        try {
            this.conectar();
                ...
                String consulta = "INSERT INTO detalle_proveedor  "
                        + "(proveedor_id_proveedor,producto_id_ingrediente,estado) " 
                        +" VALUES (?,?,'Activo') ";
                
            PreparedStatement pstm = this.conection.prepareStatement(consulta);
            pstm.setLong(1, detalleProveedorVO.getProveedorVO().getIdProveedor());
            pstm.setLong(2, detalleProveedorVO.getSuministroVO().getIdSuministro());
    ...
    public int actualizar(DetalleProveedorVO detalleProveedorVO) throws Exception {
      int resultado = -1;
        try {
            ...
            pstm.setLong(1, detalleProveedorVO.getSuministroVO().getIdSuministro());
...
//                detalleProveedorVO.getSuministroVO().setIdProducto(rs.getLong(1));
//                detalleProveedorVO.getSuministroVO().setNombre(rs.getString(2));
//                detalleProveedorVO.getSuministroVO().setTipo(rs.getString(3));
//                detalleProveedorVO.getSuministroVO().setCantidad(rs.getInt(4));
//                detalleProveedorVO.getSuministroVO().setEstado(rs.getString(5));
...
//                detalleProveedorVO.getProveedorVO().setTelefono(rs.getString(9));
    ...
    public   ArrayList<DetalleProveedorVO> consultarProveedores(long idProducto) throws Exception {
        ...
        try {
            ...
            String consulta = "SELECT pv.razon_social FROM " 
                    +"producto p INNER JOIN detalle_proveedor dp ON dp.producto_id_ingrediente = p.id_producto " 
    ...
    public  ArrayList<DetalleProveedorVO> consultarProductos(long idProveedor) throws Exception {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                detalleProveedorVO.getSuministroVO().setNombre(rs.getString(1));
    ...
    @Override
    public ArrayList<DetalleProveedorVO> listarDetalleSuministroProveedorPorId(long idSuministro) throws Exception {
        ArrayList<DetalleProveedorVO> lista = new ArrayList<DetalleProveedorVO>();
        DetalleProveedorVO detalleProveedorVO;
        int c = 0;
        try {
            ...
            sql.append(" SELECT dsp.id_detalle_proveedor, dsp.proveedor_id_proveedor, "
                    + " dsp.producto_id_ingrediente,dsp.estado "
                    + " FROM detalle_proveedor dsp "
                    + " WHERE dsp.producto_id_ingrediente = ? ");
            ...
            while (rs.next()) {
                ...
                detalleProveedorVO.setIdDetalleProveedor(rs.getInt(c++));
                detalleProveedorVO.getProveedorVO().setIdProveedor(rs.getInt(c++));
                detalleProveedorVO.getSuministroVO().setIdSuministro(rs.getLong(c++));
                detalleProveedorVO.setEstado(rs.getString(c++));
                
                lista.add(detalleProveedorVO);
            }

        } catch (Exception e) {
            System.out.println("DetalleProveedorDAOMS : se presento un error al listar: " + e.getMessage());
