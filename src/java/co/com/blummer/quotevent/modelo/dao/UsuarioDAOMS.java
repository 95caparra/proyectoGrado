...
public class UsuarioDAOMS extends ConexionMySQL implements UsuarioDAO {
    ...
    public int insertar(UsuarioVO usuarioVO) throws Exception {
        ...
        try {
            ...
            pstm.setLong(1, usuarioVO.getEmpleadoVO().getnIdentificacionEmpleado());
    ...
    public ArrayList<UsuarioVO> listar() throws Exception {
        ...
        try {
            this.conectar();
            String consulta = "SELECT U.id_usuario,U.empleado_n_identificacion,E.nombre, E.apellido,C.nombre,U.usuario "
                    + "FROM (usuario U INNER JOIN empleado E ON U.empleado_n_identificacion = E.n_identificacion) "
                    + "INNER JOIN cargo C ON E.cargo_id_cargo = C.id_cargo "
            ...
            while (rs.next()) {
                ...
                usuarioVO.getEmpleadoVO().setnIdentificacionEmpleado(rs.getLong(2));
                usuarioVO.getEmpleadoVO().setNombre(rs.getString(3));
                usuarioVO.getEmpleadoVO().setApellido(rs.getString(4));
                usuarioVO.getEmpleadoVO().getCargoVO().setNombreCargo(rs.getString(5));
                usuarioVO.setUsuario(rs.getString(6));
    ...
    public UsuarioVO consultarPorId(int idUsuario) throws Exception {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                usuarioVO.getEmpleadoVO().setnIdentificacionEmpleado(rs.getLong(3));
