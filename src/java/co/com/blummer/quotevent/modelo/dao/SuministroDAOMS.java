...
public class SuministroDAOMS extends ConexionMySQL implements SuministroDAO {
    ...
    public ArrayList<SuministroVO> listarSuministros() throws Exception {
        ...
        try {
            ...
            sql.append(" SELECT ");
            sql.append("   s.id_suministro, ");
            sql.append("	s.nombre, ");
            sql.append("	s.foto, ");
            sql.append("	s.cantidad, ");
            sql.append("	s.cantidad_minima, ");
            sql.append("	s.medida, ");
            sql.append("	mp.nombre, ");
            sql.append("	s.precio_unidad,  ");
            sql.append("	s.observaciones, ");
            sql.append("	s.estado ");
            sql.append(" FROM   suministro s ");
            sql.append(" JOIN medida_producto mp ON mp.id_medida = s.medida ");
            ...
            while (rs.next()) {
                ...
                suministroVO.getMedidaProductoVO().setNombreMedida(rs.getString(c++));
    ...
    public long ultimoId() throws Exception {
        ...
        try {
            this.conectar();
            String consulta = "SELECT id_suministro "
                    + "FROM suministro ORDER BY 1 DESC LIMIT 1 ";
            ...
            System.out.println("SuministroDAOMS: Se presento un ERROR al consultar el id del producto "
                    + e.getMessage());
            throw e;
        } finally {
            this.desconectar();
    ...
    @Override
    public void consultarCantidadesMinimas() throws Exception {
