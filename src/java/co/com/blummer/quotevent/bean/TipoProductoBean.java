...
public class TipoProductoBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            id = selectedTipoProducto.getIdTipoProducto();
            
            nombre = ((TipoProductoVO) event.getObject()).getNombre();

            boolean bandera = tipoProductoService.validarExistencia(id);

            tipoProductoVO = new TipoProductoVO();

            tipoProductoVO.setIdTipoProducto(id);
            tipoProductoVO.setNombre(nombre);

            if (bandera == true) {
                tipoProductoService.actualizar(tipoProductoVO);
                FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            } else {
                tipoProductoService.insertar(tipoProductoVO);
                FacesContext.getCurrentInstance().addMessage(null,
