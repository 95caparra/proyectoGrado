...
public class ProveedorBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            idProveedor = selectedProveedor.getIdProveedor();

            razonSocialProveedor = ((ProveedorVO) event.getObject()).getRazonSocial();
            direccionProveedor = ((ProveedorVO) event.getObject()).getDireccion();
            telefonoProveedor = ((ProveedorVO) event.getObject()).getTelefono();
            correoProveedor = ((ProveedorVO) event.getObject()).getCorreo();
            estadoProveedor= ((ProveedorVO) event.getObject()).getEstado();

            boolean bandera = proveedorService.validarExistencia(idProveedor);

            proveedorVO = new ProveedorVO();
            
            proveedorVO.setIdProveedor(idProveedor);
            proveedorVO.setRazonSocial(razonSocialProveedor);
            proveedorVO.setDireccion(direccionProveedor);
            proveedorVO.setTelefono(telefonoProveedor);
            proveedorVO.setCorreo(correoProveedor);
            proveedorVO.setEstado(estadoProveedor);
            
            if (bandera == true) {
                proveedorService.actualizar(proveedorVO);
            }else{
                proveedorService.insertar(proveedorVO);
            }
    ...
    /**
     * @param correoProveedor the correoProveedor to set
     */
