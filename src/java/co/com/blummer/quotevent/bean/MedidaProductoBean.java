...
@ManagedBean(name = "medidaProductoBean")
@ViewScoped
public class MedidaProductoBean implements Serializable {
    ...
    public void onAddNew() {
        try {
            MedidaProductoVO medidaProductoVO = new MedidaProductoVO();
            medidas = medidaService.listar();
            medidas.add(medidaProductoVO);
        } catch (Exception e) {
        }
    }
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            id = selectedMedida.getMedidaProducto();

            nombre = ((MedidaProductoVO) event.getObject()).getNombreMedida();

            boolean bandera = medidaService.validarExistencia(id);

            medidaProductoVO = new MedidaProductoVO();

            medidaProductoVO.setMedidaProducto(id);
            medidaProductoVO.setNombreMedida(nombre);

            if (bandera == true) {
                medidaService.actualizar(medidaProductoVO);
                FacesContext.getCurrentInstance().addMessage(null,
                        new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            } else {
                medidaService.insertar(medidaProductoVO);
                FacesContext.getCurrentInstance().addMessage(null,
