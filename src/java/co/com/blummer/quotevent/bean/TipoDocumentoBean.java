...
public class TipoDocumentoBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            nombre = ((TipoDocumentoVO) event.getObject()).getNombreDocumento();
            abreviatura = ((TipoDocumentoVO) event.getObject()).getAbreviatura();

            boolean bandera = tipoDocumentoService.validarExistencia(id);

            tipoDocumentoVO = new TipoDocumentoVO();

            tipoDocumentoVO.setIdTipoDocumento(id);
            tipoDocumentoVO.setNombreDocumento(nombre);
            tipoDocumentoVO.setAbreviatura(abreviatura);

            if (bandera == true) {
                tipoDocumentoService.actualizar(tipoDocumentoVO);
                FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            } else {
                tipoDocumentoService.insertar(tipoDocumentoVO);
                FacesContext.getCurrentInstance().addMessage(null,
