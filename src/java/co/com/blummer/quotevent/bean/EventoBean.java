...
public class EventoBean implements Serializable {
    ...
    public void enviarCorreo(Integer id) throws Exception {
        eventoVO = eventoService.consultarPorId(id);

        generarPDF = new GenerarPDF();
        mail = new Mail();

        Integer idPaquete = eventoVO.getSolicitudVO().getPaqueteVO().getIdPaquete();
        ...
        for (DetallePaqueteProductoVO dpp : lista) {
            productos.add(productoService.consultarPorId(dpp.getProductoVO().getIdProducto()));
        }

        String rutaImagen = Path.getPathArchivos() + "\\" + "imagenesProductos" + "\\" + eventoVO.getSolicitudVO().getPaqueteVO().getFoto();

        String salida = Path.getPathArchivos() + "\\" + "imagenesProductos" + "\\" + "EVENTO.pdf";

        generarPDF.createPDF("Cotización Evento", eventoVO, (ArrayList<ProductoVO>) productos, "Blumer eventos", rutaImagen, salida);

        mail.envioCorreoCliente(eventoVO.getClienteVO().getCorreo(), "Evento", "Evento Negociado", salida);
