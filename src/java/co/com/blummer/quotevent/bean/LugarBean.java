...
public class LugarBean implements Serializable {
    ...
    private Integer cantidadPersonaMax;
    private String direccion;
    private String ubicacion;
    private String contacto;
    private String telefonoContacto;
    private Integer selectedIdCiudad;
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            id = selectedLugar.getIdLugar();
            
            nombre = ((LugarVO) event.getObject()).getNombre();
            descripcion = ((LugarVO) event.getObject()).getDescripcion();
            cantidadPersonaMax = ((LugarVO) event.getObject()).getCantidadMaxPersonas();
            direccion = ((LugarVO) event.getObject()).getDireccion();
            ubicacion = ((LugarVO) event.getObject()).getUbicacion();
            contacto = ((LugarVO) event.getObject()).getContacto();
            telefonoContacto = ((LugarVO) event.getObject()).getNombre();
            //selectedIdCiudad = selectedCiudad;
            
            boolean bandera = lugarService.validarExistencia(id);
            
            lugarVO = new LugarVO();
            
            lugarVO.setIdLugar(id);
            lugarVO.setNombre(nombre);
            lugarVO.setDescripcion(descripcion);
            lugarVO.setCantidadMaxPersonas(cantidadPersonaMax);
            lugarVO.setDireccion(direccion);
            lugarVO.setUbicacion(ubicacion);
            lugarVO.setContacto(contacto);
            lugarVO.setTelefonoContacto(telefonoContacto);
            lugarVO.getCiudadVO().setIdCiudad(selectedIdCiudad);
            
            if (bandera == true) {
                lugarService.actualizar(lugarVO);
                FacesContext.getCurrentInstance().addMessage(null,
                        new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            }else{
                lugarService.insertar(lugarVO);
                FacesContext.getCurrentInstance().addMessage(null,
    ...
    /**
     * @return the cantidadPersonaMax
     */
    public Integer getCantidadPersonaMax() {
        return cantidadPersonaMax;
    }

    /**
     * @param cantidadPersonaMax the cantidadPersonaMax to set
     */
    public void setCantidadPersonaMax(Integer cantidadPersonaMax) {
        this.cantidadPersonaMax = cantidadPersonaMax;
    }
    ...
    /**
     * @return the selectedIdCiudad
     */
    public Integer getSelectedIdCiudad() {
        return selectedIdCiudad;
    }

    /**
     * @param selectedIdCiudad the selectedIdCiudad to set
     */
    public void setSelectedIdCiudad(Integer selectedIdCiudad) {
        this.selectedIdCiudad = selectedIdCiudad;
    }
