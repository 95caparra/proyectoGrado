...
public class RolBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            nombre = ((RolVO) event.getObject()).getNombreRolString();

            boolean bandera = rolService.validarExistencia(id);

            rolVO = new RolVO();

            rolVO.setIdRol(id);
            rolVO.setNombreRolString(nombre);

            if (bandera == true) {
                rolService.actualizar(rolVO);
                FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            } else {
                rolService.insertar(rolVO);
                 FacesContext.getCurrentInstance().addMessage(null,
