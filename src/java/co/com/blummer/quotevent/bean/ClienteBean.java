...
public class ClienteBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            idCliente = selectedClienteVO.getNumeroIdentificacion();

            nombreCliente = ((ClienteVO) event.getObject()).getNombre();
            apellidoCLiente = ((ClienteVO) event.getObject()).getApellido();
            telefonoCliente = ((ClienteVO) event.getObject()).getTelefono();
            celularCliente = ((ClienteVO) event.getObject()).getCelular();
            direccionCliente = ((ClienteVO) event.getObject()).getDireccion();
            correoCliente = ((ClienteVO) event.getObject()).getCorreo();

            boolean bandera = clienteService.validarExistenciaCliente(idCliente);

            clienteVO = new ClienteVO();

            clienteVO.setNumeroIdentificacion(idCliente);
            clienteVO.getDocumentoVO().setIdTipoDocumento(selectedTiposDocumento);
            clienteVO.setNombre(nombreCliente);
            clienteVO.setApellido(apellidoCLiente);
            clienteVO.setTelefono(telefonoCliente);
            ...
            clienteVO.setCorreo(correoCliente);

            if (bandera == true) {
                clienteService.actualizar(clienteVO);
                FacesContext.getCurrentInstance().addMessage(null,
                        new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Cliente actualizado"));
            } else {
                clienteService.insertar(clienteVO);
                FacesContext.getCurrentInstance().addMessage(null,
