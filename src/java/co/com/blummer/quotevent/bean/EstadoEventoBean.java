...
@ManagedBean(name = "estadoEventoBean")
@ViewScoped
public class EstadoEventoBean implements Serializable {
    ...
    public void onAddNew() {
        try {
            EstadoEventoVO estadoEventoVO = new EstadoEventoVO();
            estadosEvento = estadoEventoService.listar();
            estadosEvento.add(estadoEventoVO);
        } catch (Exception e) {
        }
    }
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            id = selectedEstadoEvento.getIdEstadoEvento();
            
            nombre = ((EstadoEventoVO) event.getObject()).getNombre();
            
            boolean bandera = estadoEventoService.validarExistencia(id);
            
            estadoEventoVO = new EstadoEventoVO();
            
            estadoEventoVO.setIdEstadoEvento(id);
            estadoEventoVO.setNombre(nombre);
            
            if (bandera == true) {
                estadoEventoService.actualizar(estadoEventoVO);
                FacesContext.getCurrentInstance().addMessage(null,
                        new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            }else{
                estadoEventoService.insertar(estadoEventoVO);
                FacesContext.getCurrentInstance().addMessage(null,
