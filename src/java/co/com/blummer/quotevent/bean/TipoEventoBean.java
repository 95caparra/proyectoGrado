...
public class TipoEventoBean implements Serializable {
    ...
    public void onRowEdit(RowEditEvent event) {
        try {
            ...
            id = selectedTipoEvento.getIdTipoEvento();
            
            nombre = ((TipoEventoVO) event.getObject()).getNombreTipoEvento();

            boolean bandera = tipoEventoService.validarExistencia(id);

            tipoEventoVO = new TipoEventoVO();

            tipoEventoVO.setIdTipoEvento(id);
            tipoEventoVO.setNombreTipoEvento(nombre);

            if (bandera == true) {
                tipoEventoService.actualizar(tipoEventoVO);
                FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Se actualizo"));
            } else {
                tipoEventoService.insertar(tipoEventoVO);
                FacesContext.getCurrentInstance().addMessage(null,
